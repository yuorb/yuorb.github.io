...
对于个例界发话者「我」除外的人称，若以单辅音表示，则可区分同一范畴下该人称的两个分离（即各异）实例，这是通过将该辅音的其一实例放置于列位 1，另一放置于列位 4 实现的，如 püwüp [ma/BEN/DAT-ma/BEN/DAT] 「给予他/她且给予（别的）他/她」、zäwiez [mi/INS-mi/TRA] 「使用它且为惠（别的）它」
