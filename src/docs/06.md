...
除了第 5 章中详细介绍的构形词列位 VIII 中的五个动词范畴外，还有两个适用于动词构形词的形态范畴——语为与言据。列位 IX 中的元音性 `Vk` 词缀指示这些范畴。注意，在第四章中，可见列位 IX 用于名词的`Vc`格词缀。动词的`Vk`词缀与`Vc`词缀使用相同的元音形式——列位 IX 词缀是`Vc`还是`Vk`由该词的音节重读所决定。如果单词有尾重读（即重读在最后一个音节上），列位 IX 的值为 `Vk`；否则为 `Vc`。下文第 6.1 节解释了语为与言据的范畴，第 6.1.3 节提供了表示这些范畴的列位 IX `Vk` 词缀值。
...
<dl class="gloss">
  <dt>Wuišštilkříu</dt>
  <dd>[默认Ca]-词干3/PRC/NEG₁/4-‘SARS-CoV-2.virus’-ADI₁/4-<b>HOR</b></dd>
</dl>
<dl class="gloss">
  <dt>wopňuivẓe!</dt>
