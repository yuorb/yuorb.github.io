...
Columns 2 and 3 of the above table are used if the Slot IV `Vr` value is default - a - (STA/BSC/EXS) and the Slot VI `Ca` affix is one of eight specific `Ca` values shown for Slot II as previously described in [Sec. 3.10](03#Sec3_10).
...
In the absence of a convenient semantic root, or as an alternative to using Affixual Adjuncts (see [Sec. 8.1](08#Sec8_1)), a special formative structure, the Specialized `Cs` -Root , exists to raise a `VxCs` affix to the level of a root. This formative structure operates like a standard formative except that Slots II and IV take specialized `Vv` and `Vr` forms and the Slot III `Cr` form is replaced by the `Cs` -form of a `VxCs` affix. The specifics are as follows:
