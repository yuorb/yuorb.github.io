...
Note that such specialized `Cs` -roots do not take Slot I `Cc` values indicating Slot IV/VI a+ `Ca` shortcuts but can take the four Slot I `Cc` values that indicate concatenation only (see [Sec. 10.1](10#Sec10_1) on Concatenation): **h**-, **hw**-, plus these two forms with ultimate stress.
...
-P- | <tooltip label="-P-">POSITION/LOCATION AT 0 / +Y / 0</tooltip> {.indent}
<div class="indent">
  <dl>
    <dt>Meaning</dt>
...
-XT- | <tooltip label="-XT-">EXTERNAL QUASI-LINEAR EDGE OF AN ENTITY</tooltip> {.indent}
